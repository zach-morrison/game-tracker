...
function Completed({completedGames, removeGame}) {
        ...
        {completedGames && completedGames.length > 0 && completedGames.map((game, i) => {
          return (
            <tr key={i}  className="border-y-2 border-gray-800">
              ...
              <td className="text-center"><button onClick={() => removeGame(game.name, 'completed')} className="font-bold text-red-600">X</button></td>
            </tr>
          )
        })}
        </tbody>
